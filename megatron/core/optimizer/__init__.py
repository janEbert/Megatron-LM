...
def _get_muon_fsdp_kwargs(config: OptimizerConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "fsdp_defer_partial_distributed_ns_under_gather": getattr(
            config, "muon_fsdp_defer_partial_distributed_ns_under_gather", False
        ),
        ...
        "fsdp_approx_local_boundary_global_norm_scale": getattr(
            config, "muon_fsdp_approx_local_boundary_global_norm_scale", False
        ),
        "fsdp_approx_local_boundary_foreach_norm": getattr(
            config, "muon_fsdp_approx_local_boundary_foreach_norm", False
        ),
        "fsdp_approx_local_boundary_flat_norm_all_reduce": getattr(
            config, "muon_fsdp_approx_local_boundary_flat_norm_all_reduce", False
        ),
        ...
        "fsdp_foreach_pre_ns": getattr(config, "muon_fsdp_foreach_pre_ns", False),
        "fsdp_foreach_weight_update": getattr(config, "muon_fsdp_foreach_weight_update", False),
        "fsdp_foreach_gather_weight_update": getattr(
            config, "muon_fsdp_foreach_gather_weight_update", False
        ),
    }
