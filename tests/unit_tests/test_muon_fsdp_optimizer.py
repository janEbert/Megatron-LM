...
def make_sharded_grad(full_rows, cols, dp_size, dp_rank, seed=42, device='cuda'):
    """Create full grad and return (full_grad, local_shard, shard_rows).

    The full gradient is split into dp_size equal-sized shards along dim-0.
    When full_rows is not divisible by dp_size, the last shard is zero-padded
    so all shards have the same size (mimicking FSDP bucket padding).
    """
    ...
    return full_grad, local_shard, shard_rows
...
class TestPhase1ChainedOptimizer:
    ...
    def test_phase1_chained_optimizer_step_protocol(self, model_auto_sync):
        """Verify step() call order: finish_grad_sync -> inner.step -> install_optimized_model_weights.
        ...
        # Verify ordering: finish_grad_sync (if called) before step, step before install
        if not model_auto_sync:
            # Check call order through mock's call tracking
            all_calls = []
    ...
    def test_phase1_get_mfsdp_models_success(self):
        ...
        # hasattr checks for finish_grad_sync and module
...
class TestPhase2Orthogonalize:
    ...
    def _make_fsdp_muon(self, dp_group=None, **kwargs):
        ...
        # Create with a small dummy param (won't be used directly in orthogonalize tests)
    ...
    def test_phase2_orthogonalize_correctness_plain_tensors(self, shape):
        """Core invariant: allgather -> NS -> reshard == NS(full_G)[shard_slice].

        All ranks seed the same full gradient, each holds its row-shard.
        FSDPZeROTensorParallelMuon.orthogonalize(shard) must match
        TensorParallelMuon.orthogonalize(full_G)[shard_slice].
        """
        M, C = shape
        ...
        full_grad, local_shard, shard_rows = make_sharded_grad(
            M, C, dp_size, dp_rank, seed=42
        )

        # Reference: orthogonalize full gradient, then extract shard
        ref_full = make_reference_orthogonalize(full_grad)
        start = dp_rank * shard_rows
        end = min(start + shard_rows, M)
        ref_shard = ref_full[start:end]
        # Zero-pad if needed (last rank)
        if ref_shard.shape[0] < shard_rows:
            pad = torch.zeros(
                shard_rows - ref_shard.shape[0], C, device='cuda', dtype=torch.float32
            )
            ref_shard = torch.cat([ref_shard, pad], dim=0)

        # FSDP Muon with dp_group = WORLD
        p = torch.randn(M, C, device='cuda')  # plain tensor param
    ...
    def test_phase2_orthogonalize_padding_correctness(self, M):
        ...
        The last rank's padding rows must be zero, real rows must match reference.
        """
        C = 16
        dp_group = torch.distributed.group.WORLD
        dp_size = torch.distributed.get_world_size()
        ...
        # Check real rows match
        real_rows = end - start
        torch.testing.assert_close(result[:real_rows], ref_shard, atol=1e-5, rtol=1e-4)

        # Check padding rows are zero (if any)
        if real_rows < shard_rows:
            pad_result = result[real_rows:]
            assert torch.all(pad_result == 0), (
                f"Padding rows should be zero, got max={pad_result.abs().max().item()}"
            )
    ...
    def test_phase2_orthogonalize_dtensor_output_invariants(self, shape):
        ...
        # Wrap local_shard as a Shard(0) DTensor
        # For DTensor, the global shape is declared as (shard_rows * dp_size, C) to match
        ...
        # Create param as DTensor too (FSDPZeROTensorParallelMuon uses p.shape[0] for DTensor path)
        ...
        # Mathematical correctness: compare to reference
        ref_full = make_reference_orthogonalize(full_grad[:padded_rows])
        start = dp_rank * shard_rows
    ...
    def test_phase2_full_step_cycle_plain_tensors(self, shape):
        ...
        Runs 3 complete optimization steps through the OrthogonalizedOptimizer.step()
        path, testing the full chain: momentum -> Nesterov -> orthogonalize -> p.add_().
        """
        M, C = shape
        dp_group = torch.distributed.group.WORLD
        dp_size = torch.distributed.get_world_size()
        ...
        # Create a parameter as the shard this rank owns
        torch.manual_seed(42)
        p = torch.nn.Parameter(torch.randn(shard_rows, C, device='cuda'))
        initial_weight = p.data.clone()
        ...
        for step_i in range(3):
            # Simulate a gradient (seeded per step so it changes)
        ...
        # Verify weights change at each step (momentum accumulation)
    ...
    def test_phase2_full_step_cycle_dtensors(self, shape):
        ...
        # Create param as Shard(0) DTensor
        torch.manual_seed(42)
        p_local = torch.randn(shard_rows, C, device='cuda')
        ...
        for step_i in range(3):
            ...
            # This must not raise DTensor dispatch errors
            optimizer.step()
            optimizer.zero_grad(set_to_none=True)

        # Verify param was updated
        final_local = p_dtensor.data.to_local()
        assert not torch.equal(final_local, initial_local), (
...
class TestPhase3FactoryIntegration:
    """Phase 3 tests for get_megatron_fsdp_muon_optimizer() factory."""
    ...
    @pytest.mark.parametrize(
        "strategy", ['no_shard', 'optim', 'optim_grads', 'optim_grads_params']
    )
    def test_phase3_factory_dispatches_correct_muon_cls(self, strategy):
        """Verify factory dispatches TensorParallelMuon for no_shard, FSDPZeROTensorParallelMuon otherwise.

        Uses mocks to avoid needing a real FSDP-wrapped model for the dispatch logic.
        """
        from megatron.core.optimizer import OptimizerConfig
        from megatron.core.optimizer.muon import get_megatron_fsdp_muon_optimizer
        from megatron.core.process_groups_config import ProcessGroupCollection

        # Create a minimal model chunk with the required config attributes
        ...
        # Create real 2D linear params (Muon requires 2D)
        ...
        # Set ddp_config for sharding strategy
        ...
        # Mock get_megatron_optimizer to avoid full FSDP setup for Adam
        with patch('megatron.core.optimizer.muon.get_megatron_optimizer') as mock_get_opt:
            mock_adam = MagicMock()
            mock_adam.chained_optimizers = [MagicMock()]
            mock_get_opt.return_value = mock_adam

            # Mock _get_mfsdp_models
            with patch('megatron.core.optimizer.muon._get_mfsdp_models') as mock_mfsdp:
                mock_mfsdp.return_value = [MagicMock()]

                result = get_megatron_fsdp_muon_optimizer(
                    config=config,
        ...
        # Check the base optimizer class
    ...
    def test_phase3_factory_expert_dp_group(self):
        ...
        # Non-expert linear param
        linear_weight = torch.nn.Parameter(torch.randn(32, 16, device='cuda'))
        linear_weight.requires_grad = True
        linear_weight.is_embedding_or_output_parameter = False

        # Expert linear param (has 'experts' in name)
        ...
        # _get_param_groups needs to produce groups with is_expert_parallel
        # We patch it to return controlled groups
        ...
        with patch('megatron.core.optimizer.muon._get_param_groups', side_effect=fake_get_param_groups):
            with patch('megatron.core.optimizer.muon.get_megatron_optimizer') as mock_get_opt:
                mock_adam = MagicMock()
                mock_adam.chained_optimizers = [MagicMock()]
                mock_get_opt.return_value = mock_adam
        ...
        # Should have at least 3 optimizers: non-expert Muon, expert Muon, Adam
        ...
        # First optimizer: non-expert Muon -> dp_group should be dp_cp
        ...
        # Second optimizer: expert Muon -> dp_group should be expt_dp
